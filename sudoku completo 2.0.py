...
# Activar secuencias ANSI en Windows
if os.name == 'nt':
    os.system('')
...
def imprimir_tablero(tablero, modo='numerico', dificultad='facil'):
    ...
    # Selección de color y marco según dificultad
    if dificultad == 'facil':
        color = COLOR_AZUL
        marco = '*'
    else:
    ...
    print(f"\n    {color}" + "   ".join(letras_col) + f"{RESET}")
    print(f"  {color}" + marco * 17 + f"{RESET}")
    for i, fila in enumerate(tablero):
        fila_convertida = [valores[num - 1] if num != 0 else ' ' for num in fila]
        print(f"{i+1} {color}{marco} " + ' | '.join(fila_convertida) + f" {marco}{RESET}")
        print(f"  {color}" + marco * 17 + f"{RESET}")
...
def main():
    ...
    while True:
        ...
        print("2️⃣ Salir")
        print("3️⃣ Ver mejores puntajes")
